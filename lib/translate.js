...
module.exports = function() {
  var opts, text, sourceLang, targetLang, callback;
  ...
  return request({
    ...
    function Translation(json) {
      _.extend(this, json);
      this.translation = this.sentences.reduce(function (combined, trans) {
        return combined + (trans.trans || '')
      }, '');
    }
    ...
    var translation = new Translation(json);

    callback(translation);
    return translation;

  }).catch(function(err) {
    var newError = new Error('Couldn\'t retrieve a valid JSON response. Perhaps the API has changed, please let us know.');
    newError.data = data;
